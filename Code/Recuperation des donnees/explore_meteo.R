...
recuperer_meteo <- function()
{
  ...
  # on repasse les NA pour garder les mieux remplis sur les corr?l?es
  valNA <- sapply(df,function(x) sum(is.na(x)))
  txNA  <- valNA / nrow(df)*100
  
  # td et tc corr?l?s on garde tc temp?rature plutot que point de ros?e
  # tc et u corr?l?s on garde tc temp?rature plutot que humidit?
  # cod_tend et tend + tend24 corr?l?s on garde cod_tend type de tendance barom?trique, moins de modalit?s et idem en NA
  # hnuage1 corr?l? avec hbas cl ctype1 on garde cl 
  ...
  # nnuage1 et n corr?l?s on garde n n?bulosit? totale avec moins de NA
  # n et nbas corr?l?s on garde n n?bulosit? totale 
  # ww corr?l? avec cl cm et ch on garde ww temps pr?sent avec moins de NA
  # rr1 corr?l? avec rr3 rr6 rr12 et rr24 on garde rr3 pr?cipitations 3 dernieres heures avec moins de NA
  # rafper corr?l? avec ff et raf10 on garde ff vitesse vent moyen
